...
void pair_solver::solveTranslationICP()
{
  ...
  _T_translation = chris_ICP::best_fit_transform(src, dst);
  _R_initCam2Robot = _T_translation.rowRange(0,3).colRange(0,3);
  std::cout<<"transformation t_c02b"<<std::endl<<_T_translation<<endl;
  std::cout<<"rotation R_c02b"<<std::endl<<_R_initCam2Robot<<endl;
}
...
void pair_solver::solveOrientationSDP()
{
  ...
  cout<<"src : "<<src.rows<<","<<src.cols<<endl;
  cout<<"dst : "<<dst.rows<<","<<dst.cols<<endl;
  //_R_cam2e = chris_ICP::solve_Rotation_SVD(src.t(), dst.t());2018 version
  //std::cout<<"_R_cam2e matrix"<<std::endl<<_R_cam2e<<endl; 2018 version
  _R_e2cam = chris_ICP::solve_Rotation_SVD(src.t(), dst.t());
  std::cout<<"_R_e2cam matrix"<<std::endl<<_R_e2cam<<endl; //2019 version
  //validation
  double error = math_helper::FrobeniusNorm(dstV[1] - _R_e2cam * srcV[1]);
