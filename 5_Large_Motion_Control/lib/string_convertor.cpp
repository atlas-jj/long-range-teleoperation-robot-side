...
void string_convertor::printOutStdVector_f(std::vector<float> v)
{
 //cout<<"print out v:"<<endl;
 for(int i=0;i<v.size();i++)
 {
...
void string_convertor::printStrArray(string *strs)
{
...
std::vector<double> string_convertor::split2double(const std::string &s, char delim)
{
  std::vector<double> elems;
  std::stringstream ss(s);
  std::string number;
  while(std::getline(ss, number, delim)) {
      elems.push_back(stod(number));
  }
  return elems;
}

std::vector<std::string>  string_convertor::safeRemoveStringVec(std::vector<std::string> input, int removeIndex)
{
  std::vector<std::string> newStrs;
  for(size_t i=0;i<input.size();i++)
    if(i!=removeIndex)
      newStrs.push_back(input[i]);
  return newStrs;
}
